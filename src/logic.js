...
const trimAndCompare = (newState, firstTime) => {
  ...
  if (changedStatus) {
    currentState.status = { ...currentState.status, ...changedStatus };
...
const ready = (updater, interval) => {
  ...
  return sdk
    ...
    .then(data => {
      ...
      if (sdk.platform === 'gome') {
        sdk.onStatusChanged(trimAndCompare);
      }
    })
...
const control = (function() {
  ...
  const retryControl = async function(
    cmd,
    { retryCount = 0, errorCode = null, timeout = 0 } = {}
  ) {
    ...
    return (async function(execId) {
      ...
      for (let i = 0; i < retryCount + 1; i++) {
        try {
          const data = await sdk.setDeviceStatus(cmd);
          if (uuid !== execId) {
