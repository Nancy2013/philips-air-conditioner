 ...
 * @LastEditTime: 2022-08-18 14:43:20
...
const timer = {
  ...
  getters: {
    ...
    getSleepList: state => {
      // 查询睡眠定时
      const getSleepVals = actions => {
        const { sleep_diy } = actions;
        const { params, vals } = JSON.parse(sleep_diy);
        const sleepVals = [];
        const [key1, key2] = params;
        vals.forEach((v, index) => {
          if (index % 6 === 0) {
            sleepVals.push({
              [key1]: vals[index][0].val,
              [key2]: vals[index][1].val,
            });
          }
        });
        console.log('------getSleepVals-------', sleepVals);
        return sleepVals;
      };

      const { timerList = [] } = state;
      const timers = timerList.filter(v => v.name.includes('sleep_diy'));
      timers.forEach(v => {
        const { data = '{}' } = v;
        const dataJson = JSON.parse(data);
        const { controlInfo } = dataJson;
        const payload = controlInfo.directive;
        const actions = combine(payload.payload);

        v.action = {
          ...actions,
          sleep_diy: getSleepVals(actions),
        };
        const { weekdays } = v.timer;
        v.repeat =
          weekdays.split('').map(k => {
      ...
      console.log('-----getSleepList-----', sortTimers(timers));
      timers.sort((a, b) => (a.name > b.name ? 1 : -1));
      return timers;
    },
  },
